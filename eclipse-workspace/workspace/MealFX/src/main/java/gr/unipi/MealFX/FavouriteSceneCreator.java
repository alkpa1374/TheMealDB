...
public class FavouriteSceneCreator implements EventHandler<MouseEvent>
{
     ...
     public FavouriteSceneCreator() 
     {
        ...
        deleteFavouriteBtn.setOnMouseClicked(this);
        ...
        //Checking of the existence of "favourite.json"
        if (favourite.exists())
        {
            try {
		        ...
		        //Deserialization of JSON content of "favourite.json" into a MealInfo objects List and assignment to the application's favourite meals list (FavMealList)
                App.FavMealList = mapper1.readValue(
                    new File("favourite.json"),
                    typeFactory.constructCollectionType(List.class, MealInfo.class)
                );

                //Printing of a message to indicate successful deserialization of favourite meals
        ...
        //Checking of the existence of "cooked.json"
        if (cooked.exists())
        {
            try 
			{
                //Obtaining of TypeFactory instance from mapper2 for advanced type handling
                ...
                //Printing of message to indicate successful deserialization of cooked meals
        ...
        //Printing of message indicating cooked meals successful deserialization
    ...
    {
    	//Checking of the "Delete Favourite" button as the event source
    	if (event.getSource() == deleteFavouriteBtn) 
    	{
    	    try
    	    {
    	        ...
    	        for (MealInfo MI : MealTableView.getItems())
    	        {
    	            ...
    	            {
    	                //Removal of meal from favourite meals list
    	                App.FavMealList.remove(MI);

    	                //Presentation of custom pop-up window notifying the user the meal has been successfully deleted from favourite meals list
    	        ...
    	        //Addition of all favourite meals returned by the REST API of THEMEALDB to the table view
    	        for (MealInfo Meal : App.FavMealList) 
    	        {
    	            items.add(Meal);
    	        }
    	...
    	else if (event.getSource() == moveMealBtn) 
    	{
    	    try
    	    {
    	        ...
    	        for (MealInfo MI : MealTableView.getItems())
    	        {
    	            ...
    	            if (selectedItems.contains(MI))
    	            {
    	                //Checking if the selected meal is not already in the cooked list
    	                if (!App.CoMealList.contains(MI))
    	                {
    	                    //Removal of meal from the favourite meals list
    	                    App.FavMealList.remove(MI);

    	                    //Addition of meal to cooked meals list
    	...
    	{
    	   //Switching of cooked scene to main scene
